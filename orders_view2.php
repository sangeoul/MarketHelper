...
ESI 로딩(<span id='progress'></span> orders) 자동 새로고침(5분)<input type='checkbox' name='autorefresh' id='autorefresh' checked> <span id=refreshtimer></span><br>
...
echo("<script>\n");
echo("var USER_ACCESS_TOKEN=\"".$USER_ACCESS_TOKEN."\";\n");
echo("var USER_ID=".$_SESSION['markethelper_user_id'].";\n");

}
else{

echo("<script>\n");
echo("var USER_ACCESS_TOKEN=0;");


echo("alert('ESI 에러 발생');window.location.replace('./logout.php');\n");
...
if(USER_ACCESS_TOKEN!=0){
	...
	ESIdata.open("GET","https://esi.evetech.net/latest/characters/"+USER_ID+"/orders/?datasource=tranquility",false);
	ESIdata.setRequestHeader("Content-Type", "application/json");
	ESIdata.setRequestHeader("Authorization","Bearer "+USER_ACCESS_TOKEN);
	ESIdata.send();

    
	var GOODSLIST=returnvalue;
...
var selltablestring="<table border=1><tr><th rowspan=2>Type</th><th>My Price</th><th>Lowest Price</th><th rowspan=2>Difference</th><th rowspan=2>Alarm</th><th rowspan=2>Copy</th></tr><tr><th colspan=2>Amount</th></tr>";
...
var buytablestring="<table border=1><tr><th rowspan=2>Type</th><th>My Price</th><th>Highest Price</th><th rowspan=2>Difference</th><th rowspan=2>Alarm</th><th rowspan=2>Copy</th></tr><tr><th colspan=2>Amount</th></tr>";
